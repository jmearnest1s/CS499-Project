...
if ($stage == "") {
	...
	if (!$connection = @mysqli_connect($serverName, $username, $password)) {
		...
		$DBresult = $mysqli_query($DBsql,$connection);
		if ($DBresult === false) {
			throw new Exception("Could not execute query: " . $mysqli->error);
		}
		
		$database_list = array();
		while($EBrow = $DBresult->fetch_array(MYSQLI_NUM)) { // for each row of the resultset
			$database_list[] = $EBrow[0]; // Add db name to $db_names array
		}
	
		//$database_list = mysqli_list_dbs($connection);
		$database_index = 0;
		*/
		...
		/*while ($database_index < mysqli_num_rows($database_list)) {
				$js_block .= "\n\ntableArray[$database_index] = new Array;";
				$database_options .= "\n\n\t<option value=\"" . mysqli_db_name($database_list, $database_index) . "\">" . mysqli_db_name($database_list, $database_index) . "</option>";

				$table_list = mysqli_list_tables(mysqli_db_name($database_list, $database_index));
				$table_index = 0;

				while ($table_index < mysqli_num_rows($table_list)) {
					$js_block .= "\n\ttableArray[$database_index][$table_index] = \"new Option(\\\"" . mysqli_tablename($table_list, $table_index) . "\\\", \\\"" . mysqli_tablename($table_list, $table_index) . "\\\");\";";
					$table_index++;
		...
		$table_list = mysqli_list_tables(mysqli_db_name(array('agwiki'), $database_index));
				$table_index = 0;
		...
		<form name=\"form1\" action=\"csv_importer.php\" method=\"POST\">
			<table cellspacing=\"0\" cellpadding=\"0\" border=\"0\" width=\"55%\" align=\"center\">
				...
				<tr>
					...
					<td>
						<input name='database' value='agwiki'>
					</td>
				</tr>
				<tr>
					<td>Select table:</td>
	...
	if (!$myFile = @fopen(stripslashes($csvFile), "r")) {
		...
		if (!$connection = mysqli_connect($serverName, $username, $password)) {
			...
			$fields = mysqli_list_fields($database, $table, $connection);
			$columns = mysqli_num_fields($fields);
			
			for ($index = 0; $index < $columns; $index++) {
			     $display_block .= "\n\t\t\t\t\t\t<tr>
				 		<td colspan=\"5\" class=\"tdPreviewAssignHeader\">" . mysqli_field_name($fields, $index) . ":&nbsp;</td>
					</tr>
					<tr>
				...
				$js_field_array .= "\nfieldArray[$index] = \"" . mysqli_field_name($fields, $index) . "\";";
			}
		}
	...
	if (!$connection = mysqli_connect($serverName, $username, $password)) {
		...
		for ($index = 0; $index < $columns; $index++) {
			$fieldArray[$selectedFieldIndex][0] = mysqli_field_name($fields, $index);
					...
					if ($line >= $startLine){
						...
						for ($index = 0; $index < count($fieldArray); $index++) {
							if ((($fieldColumn[$index] != "") && ($fieldColumn[$index] != "none")) || ($useValue[$index] == "value")) {
								$sql .= "`".addslashes($fieldArray[$index][0]) . "`, ";
							}
						}
						...
						for ($index = 0; $index < count($fieldArray); $index++) {
							if ($useValue[$index] == "value") {
								$sql .= "\"" . addslashes($fieldValue[$index]) . "\", ";
							} else if (($fieldColumn[$index] != "none") && ($fieldColumn[$index] != "")) {
								$sql .= "\"" . addslashes($data[$fieldArray[$index][1]]) . "\", ";
							}
						}
						
						$sql = substr($sql, 0, -2) . ")";

						if ($update_db = mysqli_query($sql, $connection)) {
							$mysqli_log .= "Ok: $sql\n<br>\n<br>";
						} else {
							$mysqli_log .= "<strong>Failed:</strong> $sql\n<br>Reason: " . mysqli_error() . "\n<br>\n<br>";
						}
				...
				while ($data = fgetcsv($myFile, 2048, $delimiter)) {
					if ($line >= $startLine){
						...
						if ($action == "insert") {
							...
							for ($index = 0; $index < count($fieldArray); $index++) {
								if ($fieldArray[$index][0] != $primaryKey) {
									$sql .= "\"" . addslashes($data[$fieldArray[$index][1]]) . "\", ";//$sql .= "\"" . $data[$fieldArray[$index][1]] . "\", ";
								}
							}
							...
							for ($index = 0; $index < count($fieldArray); $index++) {
								...
								if ($fieldArray[$index][0] == $primaryKey) {
									if ($useValue[$index] == "column") {
										$primaryKeyValue = addslashes($data[$fieldArray[$index][1]]);//$primaryKeyValue = $data[$fieldArray[$index][1]];
									} else {
										$primaryKeyValue = addslashes($fieldValue[$index]);//$primaryKeyValue = $fieldValue[$index];
									}
								}
								...
								if ($fieldArray[$index][0] != $primaryKey) {
									if (($useValue[$index] == "column")
										...
										$sql .= $fieldArray[$index][0] . " = \"" . addslashes($data[$fieldArray[$index][1]]) . "\", ";//$sql .= $fieldArray[$index][0] . " = \"" . $data[$fieldArray[$index][1]] . "\", ";
									} else if ($useValue[$index] == "value") {
										$sql .= $fieldArray[$index][0] . " = \"" . addslashes($fieldValue[$index]) . "\", ";//$sql .= $fieldArray[$index][0] . " = \"" . $fieldValue[$index] . "\", ";
									}
								}
							}
