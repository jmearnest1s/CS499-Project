...
<head>
    ...
    <title><?php echo e(((isset($page_title))?$page_title:'Home')); ?> | <?php echo e($gnl->title); ?></title>
    <meta name="Title" Content="<?php echo e(((isset($page_title))?$page_title:'Home')); ?> | <?php echo e($gnl->title); ?>">
    ...
    <meta property="og:title" content="<?php echo e(((isset($page_title))?$page_title:'Home')); ?> | <?php echo e($gnl->title); ?>"/>
...
<?php echo $__env->make('custom.sidebar', \Illuminate\Support\Arr::except(get_defined_vars(), array('__data', '__path')))->render(); ?>
				...
				$(document).ready(function() {				
					...
					$(".post-input-field").mention({
						...
						{							
							...
							image: '/assets/front/img/<?php echo e($user->avatar); ?>'
						},
						...
						.then((willDelete) => {
						  ...
						  if (willDelete) {
								...
								$.ajax({
									...
									url: '<?php echo e(route('user.comment.delete')); ?>', 
							...
							if (urlpat.test(value)) {//.test(value)
								...
								setTimeout(function() {
									$.ajax({
										...
										url:"https://guteurls.de/ajax-online.php",//url:"https://guteurls.de/ajax.php/"+value+"?html=1&u="+value+"&r=https://agwiki.dev2.blayzer.com/urlpreview?url="+value+"&h=6458&e=&d=0&w=0",//url: "<?php echo e(route('urlpreview')); ?>?url="+value,//url: "<?php echo e(route('user.urllink.data')); ?>",
										data: {
											'u': value,
											'task':'loadBox',
												...
												setTimeout(function() {
															...
															$("#loaderimg").show();
															  ...
															  $.ajax({
																	type: "POST",
																	async: true,   // this will solve the problem
																	url:"<?php echo e(route('user.urllink.data')); ?>",
																	data: {
																		urllink: value,
																		_token: '<?php echo e(csrf_token()); ?>'
																	},
			 ...
			 $(document).on('click', '.like', function(e) {
				...
				<?php if(isset($_GET['topic'])): ?>
				 		$('.page-link').each(function() {
							...
							console.log(href);
							if (href) {
								href += (href.match(/\?/) ? '&' : '?') + 'topic=<?php echo e($_GET["topic"]); ?>';
								$(this).attr('href', href);
								console.log(href);
							}
						});
                    ...
                    swal({
                        ...
                        if (willDelete) {
                            $.ajax({
                                ...
                                url: "<?php echo e(route('user.post.delete')); ?>",
                                type: 'POST',
                                success: function(response) {
                                    if (response.success && response.success == 1) {
                    ...
                    swal({
                        ...
                        if (willDelete) {
                            $.ajax({
                                ...
                                url: "<?php echo e(route('user.share.delete')); ?>",
                                type: 'POST',
                                success: function(response) {
                                    if (response.success && response.success == 1) {
                                        $('.deletable').parent().parent().parent().parent().slideUp().remove();
                ...
                $(document).on('click', '.share', function(e) {
                    ...
                    $.ajax({
                        type: "POST",
						context: this,
                        url: "<?php echo e(route('user.share')); ?>",
                        data: {
                            post_id: post,
							group_id: group,
                            _token: '<?php echo e(csrf_token()); ?>'
                        ...
                        success: function(data) {
				            ...
				            function upload(file, type) {
                                ...
                                ajax.open("POST", "<?php echo e(route('file.store')); ?>");
                                ajax.send(formdata)
                            }
                                ...
                                if (jso.error) {
                                  ...
                                  //  $('.emojionearea-editor').attr('placeholder', 'Caption');
                                   ...
                                   // toastr.success('Uploaded Successfully. Just Share This');
                                    if (jso.type == 'image') {
                                        $('#cont').fadeIn();
                                        $('#cont div').html('<img src="<?php echo e(url("assets/front/content/")); ?>/' + jso.link + '" class="img-responsive" style="width: 100%;margin-bottom: 20px;">');
                                    } else if (jso.type == 'video') {
                                        $('#cont').fadeIn();
                                        var html = '<video id="player" playsinline controls>\n' + '<source src="<?php echo e(url("assets/front/content/")); ?>/' + jso.link + '" type="video/mp4" id="player-src">\n' + '</video>';
                                        $('#cont div').html(html);
                                        const player = new Plyr('#player');
                                    } else if (jso.type == 'audio') {
                                        $('#cont').fadeIn();
                                        var html = '<audio id="player" controls> <source src="<?php echo e(url("assets/front/content/")); ?>/' + jso.link + '" type="audio/mp3" id="player-src"> < /audio>';$('#cont div').html(html);const player=new Plyr('#player')}
                                        setTimeout(function() {
                                            $("#prog").css('width', '0%');
                                            $("#prog").text('0%');
                                            $("#prog").fadeOut();
